"""
vLLM LoRA Checkpoint Inference Script

This script automates batch inference across multiple LoRA checkpoints using vLLM. 
It supports dynamic loading of LoRA adapters on top of a base causal language model,
parses conversational message formats, and extracts reasoning/answers from generated outputs.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run vLLM inference for every checkpoint under output/sft."
    )

    parser.add_argument(
        "--model_root",
        type=str,
        default="./output/sft_models",
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="*",
        default=[], # "checkpoint-1646","checkpoint-2469"
        help="List of specific checkpoint folder names to evaluate (e.g. 'checkpoint-1000' 'final_model'). If empty, evaluates all.",
    )
    parser.add_argument(
        "--max_samples",
        type=int,
    ...
    parser.add_argument(
        ...
        default=True,
        action="store_true",
        help="Also evaluate output/sft/final_model if it exists.",
    )
...
def find_checkpoints(model_root: str, include_final_model: bool = False, only_final: bool = False) -> List[Path]:
...
def build_prompt_from_example(tokenizer, ex: Dict) -> str:
    """
    Expected data format: ex["messages"] = [..., {"role": "assistant", "content": gold}]
    The final gold assistant message is excluded during inference prompting.
    """
    prompt_messages = ex["messages"]
    filtered_messages = [msg for msg in prompt_messages if msg.get("role") in ["system", "user"]]
    
    # Validation: Extract and verify all present roles
    present_roles = {msg.get("role") for msg in filtered_messages}
    
    if present_roles != {"system", "user"}:
        raise ValueError(f"Unexpected or missing roles in messages. Found roles: {present_roles}")
...
def prepare_requests(tokenizer, data: List[Dict]) -> Tuple[List[str], List[Dict]]:
    prompts: List[str] = []
    rows: List[Dict] = []

    for ex in data:
        prompt = build_prompt_from_example(tokenizer, ex)
...
def run_one_split(
    ...
    prompts, rows = prepare_requests(tokenizer, raw_data)
    ...
    for idx, (ex, prompt, out) in enumerate(zip(rows, prompts, outputs)):
        ...
        # gold_text = str(messages[-1]["content"]).strip()
        # gold_fields = parse_gold_fields(gold_text)
...
def cleanup_llm(llm: Optional[LLM]) -> None:
    """
    Release GPU memory after evaluating each checkpoint to prevent OOM 
    when sequentially loading multiple checkpoints.
    """
    if llm is not None:
        del llm
    gc.collect()
...
def main() -> None:
    ...
    for ckpt_dir in checkpoints:
        ...
        val_done = (ckpt_output_dir / "validation_predictions.jsonl").exists() if args.validation_file else True
